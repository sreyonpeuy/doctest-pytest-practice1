...
def threes(n):
    ...
    '''
    num_to_n = list(range(n + 1))
    three = filter(lambda x: '3' in str(x), num_to_n)
    return list(three)
...
def small_words(text):
    ...
    small = filter(lambda x: len(x) < 5, all_words)
    return list(small)
...
def squares(n):
    ...
    '''
    num_to_n = range(1, n + 1)
    squared = map(lambda x: x * x, num_to_n)
    return list(squared)


def lengths(strings):
    '''
    Given a list of strings, returns a list of the lengths of
